<template>
  <view v-if="!!url" class="img-crop">
    <canvas type="2d" :canvas-id="canvasId" :id="canvasId" :style="[canvasStyles]" />
    <view class="container">
      <movable-area class="movable-area" :style="[movableAreaStyles]">
        <movable-view
            class="movable-view" :style="[movableViewStyles]"
            direction="all"
            :x="x" :y="y"
            :scale-value="scale" :scale-min="1" :scale-max="maxScale ?? 3"
            scale
            :animation="false"
            @change="handleChange"
            @scale="handleScale"
...
const cropperMaxWidth = windowWidth * .7;
const cropperMaxHeight = (windowHeight - 48) * .8;
...
const movableAreaStyles = computed(() => {
  ...
  if (rotate.value % 180 !== 0) {
    [movableAreaWidth.value, movableAreaHeight.value] = [movableAreaHeight.value, movableAreaWidth.value];
  }
...
const movableViewStyles = computed(() => {
  [movableViewWidth.value, movableViewHeight.value] = [initialImageWidth.value, initialImageHeight.value];
  if (rotate.value % 180 !== 0) {
    [movableViewWidth.value, movableViewHeight.value] = [movableViewHeight.value, movableViewWidth.value];
  }
...
watch(props, ({ url, aspect = 1, width, height }) => {
    ...
    uni.getImageInfo({
      ...
      success: (res) => {
        ...
        if (width && height) {
          ...
          const cropperScale = Math.min(originalWidth / cropperWidth.value, originalHeight / cropperHeight.value);
...
async function crop() {
  ...
  let ctx: WechatMiniprogram.CanvasRenderingContext.CanvasRenderingContext2D | UniNamespace.CanvasContext;
